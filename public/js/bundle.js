...
var displayMap = function displayMap(locations) {
  mapboxgl.accessToken = 'pk.eyJ1IjoibGlldHNha2kiLCJhIjoiY2s2OWk1eXc5MDZyNDNmbjM5cTlnbXB3NiJ9.LaFEKLE6oopaNTZkruqtsg'; //- Our map. It'll appear in an element with the "map" id
  ...
  var bounds = new mapboxgl.LngLatBounds(); // Add a marker for every location. loc.coordinates contains an array of lat and lng, because it comes from tour.locations
  ...
  locations.forEach(function (loc) {
    ...
    el.className = 'marker'; //- Create a marker in mapbox and use our own marker we just defined above. We can use mapboxgl because we appended their script on tour.pug

    new mapboxgl.Marker({
      element: el,
      anchor: 'bottom'
    }).setLngLat(loc.coordinates).addTo(map); // Make sure to add this marker to our map
    // Add a popup

    new mapboxgl.Popup({
      offset: 30,
      anchor: 'bottom'
    }).setLngLat(loc.coordinates).setHTML("<p>Day ".concat(loc.day, ": ").concat(loc.description, "</p>")).addTo(map); // Extend the empty bound object we created above to include the current location
...
var homeRedirect = function homeRedirect(time) {
      ...
      while (1) {
        switch (_context2.prev = _context2.next) {
          ...
          case 3:
            ...
            // Redirect to the homepage after 1.5 seconds
            if (res.data.status === 'Success!') {
              (0, _alerts.showAlert)('success', 'Signed up successfully! Redirecting...');
              homeRedirect(1500);
            }
        ...
        switch (_context.prev = _context.next) {
          ...
          case 4:
            ...
            if (res.data.status === 'success' || 'Success!') {
              (0, _alerts.showAlert)('success', "".concat(capitalizeFirstLetter(type), " updated successfully!"));
            }
          ...
          case 8:
            _context.prev = 8;
            _context.t0 = _context["catch"](0);
            (0, _alerts.showAlert)('error', _context.t0.response.data.message); //console.log(err.response);
...
var stripe = Stripe('pk_test_XG3QTTyEcOUxNoRULnhQIvZb'); // Note: Since we're using the same url for the API and the website, we can skip the host in the 'url' property of our axios call,
...
var mapBox = document.getElementById('map'); // Signup Form
...
var logoutButton = document.querySelector('.nav__el--logout'); // Update my email/name form

var userDataForm = document.querySelector('.form-user-data'); // Password form

var userPasswordForm = document.querySelector('.form-user-password'); // Button to book a tour in the tour page

var bookBtn = document.getElementById('book-tour'); // Display the map if the page has a div with the 'map' id
...
if (logoutButton) logoutButton.addEventListener('click', _login.logout); // Update the user name, email and/or photo

if (userDataForm) userDataForm.addEventListener('submit', function (event) {
  event.preventDefault(); // NOTE: The names of the 3 consts below must be the same as what our API expects in the /updateMe route
  ...
  form.append('photo', document.getElementById('photo').files[0]); // Our AJAX call with axios will recognize this 'form' as an object and work just as before
  ...
  regeneratorRuntime.mark(function _callee(event) {
    ...
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            event.preventDefault(); // Set the "Save password" button to "Updating..." while this function finishes its execution

            document.querySelector('.btn--save-password').textContent = 'Updating...'; // Select the three password inputs: Current password, new password and new password confirm
            ...
            passwordConfirm = document.getElementById('password-confirm').value; // Await the password update to clear the input fields after the function finishes updating the password
...
if (bookBtn) bookBtn.addEventListener('click', function (event) {
  event.target.textContent = 'Processing...'; // Note: event.target returns the element on which the event was fired. In this case, it'll be the 'Book tour now!' button
...
var alertMessage = document.querySelector('body').dataset.alert; // 2) If there's an alert, display the alert using our showAlert function

if (alertMessage) (0, _alerts.showAlert)('success', alertMessage, 20);
},{"core-js/stable":"../../node_modules/core-js/stable/index.js","regenerator-runtime/runtime":"../../node_modules/regenerator-runtime/runtime.js","./mapbox":"mapbox.js","./login":"login.js","./updateSettings":"updateSettings.js","./alerts":"alerts.js","./stripe":"stripe.js"}],"../../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
var global = arguments[3];
